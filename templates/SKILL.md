...
```
fmm_search(term: "store")                       →  smart search: exports, files, imports, call sites
fmm_search(term: "createServerFn")              →  CALL SITES: all files importing createServerFn by name
...
Universal codebase search. Use 'term' for smart search across exports, file paths, import names, and named import call sites (files that import a symbol by name from any package). Use structured filters (export, imports, depends_on, LOC) for precise queries. Combine 'term' with filters to narrow results with AND semantics. Note: depends_on uses transitive matching (full import chain), not direct-only. For direct importers only, use fmm_dependency_graph with depth=1.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `term` | string | no | Search across four dimensions: (1) codebase-defined exports (exact then fuzzy), (2) file paths, (3) external import p... |
